...
type ClusterPropertyLister interface {
	...
	List(selector labels.Selector) (ret []*apisv1beta1.ClusterProperty, err error)
	// Get retrieves the ClusterProperty from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*apisv1beta1.ClusterProperty, error)
...
type clusterPropertyLister struct {
	listers.ResourceIndexer[*apisv1beta1.ClusterProperty]
}
...
func NewClusterPropertyLister(indexer cache.Indexer) ClusterPropertyLister {
	return &clusterPropertyLister{listers.New[*apisv1beta1.ClusterProperty](indexer, apisv1beta1.Resource("clusterproperty"))}
}
