...
type ClusterPropertyLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.ClusterProperty, err error)
	// Get retrieves the ClusterProperty from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.ClusterProperty, error)
...
// List lists all ClusterProperties in the indexer.
func (s *clusterPropertyLister) List(selector labels.Selector) (ret []*v1beta1.ClusterProperty, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.ClusterProperty))
	})
	return ret, err
}

// Get retrieves the ClusterProperty from the index for a given name.
func (s *clusterPropertyLister) Get(name string) (*v1beta1.ClusterProperty, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("clusterproperty"), name)
	}
	return obj.(*v1beta1.ClusterProperty), nil
}
