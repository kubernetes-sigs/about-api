...
type ClusterPropertyLister interface {
	...
	List(selector labels.Selector) (ret []*apiv1alpha1.ClusterProperty, err error)
	// Get retrieves the ClusterProperty from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*apiv1alpha1.ClusterProperty, error)
...
type clusterPropertyLister struct {
	listers.ResourceIndexer[*apiv1alpha1.ClusterProperty]
}
...
func NewClusterPropertyLister(indexer cache.Indexer) ClusterPropertyLister {
	return &clusterPropertyLister{listers.New[*apiv1alpha1.ClusterProperty](indexer, apiv1alpha1.Resource("clusterproperty"))}
}
